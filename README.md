...
│   ├── asset1-30 *.json        #   30 个键值 asset（病机/方/注家/体质/医案/治法/瘟疫，含 19 部历代名家医案库）
...
It is based on the Yunqi theory in the seven major Suwen treatises of the *Huangdi Neijing*. The pack provides rule-based Ganzhi/Yunqi calculation (Dahan boundary), a 30-asset RAG knowledge base (incl. 19 classical case libraries), thought-layer explanations, progressive learning depth, self-evolution with privacy, and tools to export thought summaries, Anki cards, and printable reports.
...
```text
User input (natural language) → routing + onboarding (fuzzy intent handling) → Python engine (Dahan boundary) → 30-asset RAG (incl. 19 classical case libraries) → pathogenesis + **thought-layer interpretation** → reports with guiding/reflection questions → visualization → self-evolution (concept tracking + understanding feedback + privacy) → export (plain-text thought summary / Anki cards / PDF/HTML)
```
...
- 30-asset RAG knowledge base (pathogenesis, formulas, commentaries, regional, constitution + 19 classical case libraries, 901+ cases)
