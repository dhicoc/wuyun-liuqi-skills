...
│   ├── asset1-25 *.json        #   25 个键值 asset（病机/方/注家/体质/医案/治法/瘟疫，含 14 部历代名家医案库）
...
2. **RAG 知识库**（`rag-knowledge-base/`）：25 个键值 asset（含 asset9 岁图医案 + asset11-16 六部历代名家医案库 901 条 + asset17 运气瘟疫防治 + asset18 回春录医案 + asset19 张聿青医案 + asset20 吴鞠通医案 + asset21 寓意草医案 + asset22 洄溪医案 + asset23 花韵楼医案 + asset24 诊余举隅录 + asset25 许氏医案）+ 10 本公版蒸馏指南（Grep+Read）+ 35 篇文献原文
...
It is based on the Yunqi theory in the seven major Suwen treatises of the *Huangdi Neijing*. The pack provides rule-based Ganzhi/Yunqi calculation (Dahan boundary), a 25-asset RAG knowledge base (incl. 14 classical case libraries), thought-layer explanations, progressive learning depth, self-evolution with privacy, and tools to export thought summaries, Anki cards, and printable reports.
...
```text
User input (natural language) → routing + onboarding (fuzzy intent handling) → Python engine (Dahan boundary) → 25-asset RAG (incl. 14 classical case libraries) → pathogenesis + **thought-layer interpretation** → reports with guiding/reflection questions → visualization → self-evolution (concept tracking + understanding feedback + privacy) → export (plain-text thought summary / Anki cards / PDF/HTML)
```
...
- 25-asset RAG knowledge base (pathogenesis, formulas, commentaries, regional, constitution + 14 classical case libraries, 901+ cases)
