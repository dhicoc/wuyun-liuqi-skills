...
2. 用结构化 RAG 知识库补充病机、方药、注家、地域与体质知识。
3. 用子技能目录承载不同知识层级，便于 AI Agent 路由。
4. 用报告与可视化脚本输出可读结果。
5. 用自进化系统记录使用、盲区与反馈。
...
SKILL.md / 子技能 SKILL.md
  ↓
scripts/calculate_yunqi_api.py（Python 主链路）
  ↓
rag-knowledge-base/ 精确检索
  ↓
yunqi-pathogenesis/ 病机分析
  ↓
yunqi-clinical/ 养生与临床参考
  ↓
docs-generator/ 报告生成
  ↓
reports/generated/ 可视化或报告输出
  ↓
scripts/self_evolve.py 使用记录与反馈
```
...
| 目录 | 定位 |
|------|------|
| `scripts/` | 可执行工具、推算引擎、报告生成、自进化 CLI |
| `scripts/lib/` | 推算共享数据表与底层规则 |
| `rag-knowledge-base/` | RAG 结构化知识资产 |
