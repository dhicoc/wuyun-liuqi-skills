...
name: wuyun-liuqi
description: >
  五运六气（运气学）AI Agent 技能包。人类将其安装到 AI Agent 中后，Agent 就能帮助人类准确、系统、深入地理解《黄帝内经》运气学这个思想体系（天人合一、气化、中和、时间节律）。
  提供干支推算、思想层解读、概念哲学解释、导出复习材料（摘要/卡片/PDF）、自进化持续优化 + 隐私保护。
  触发词：五运六气、运气、大运、主运客运、主气客气、司天在泉、客主加临、
  太过不及、天符岁会、运气病机、运气治法、七篇大论。
  最终价值：帮助人类通过与 AI 的对话，深入理解运气学背后的宇宙观、时间观与生命观。
  面向人群：对运气学思想感兴趣的学生、希望获得理论参考的医师、进行文献研究的学者。
---
...
**对人类的意义**：当你（人类用户）在与 AI 的对话中提到运气相关话题时，本技能会让 AI 成为一个可靠的讲解者，帮助你真正理解五运六气这个思想，而不是简单给出计算结果。
...
3. 验证安装结果并向用户报告：
   ...
   ✅ 端到端验证: python scripts/verify_expansion.py + tests/full_regression_test.py（完整回归 63 项，通过 0 失败）
...
| **自进化引擎** | `self-evolve/` + `scripts/self_evolve.py` | **日志+概念追踪+理解反馈+隐私（哈希/清洗）+月报+清理**（持续优化思想讲解质量） |
## 免责声明前置文件读取顺序
...
| **`calculate_yunqi_api.py`** | **日期→完整运气JSON（大寒定年）+ 思想层/导出** | **`python scripts/calculate_yunqi_api.py today --summary --level deep --explain-concept "天人合一" --export all`** |
...
| **`self_evolve.py`** | **自进化引擎**（使用日志、概念追踪、理解反馈、隐私哈希、月报、清理） | **`python scripts/self_evolve.py stats|feedback|report|cleanup ...`** |
| **`export_thought.py`** | **思想导出**（纯文本思想摘要、Anki卡片集、PDF/HTML 打印版） | **`python scripts/export_thought.py today --format summary|cards|pdf|all`** |
