...
# 含中文数字剂量（一两/三钱/二钱半）与阿拉伯数字，二者都替换为占位符。
DOSE_PATTERNS = [
    # 阿拉伯数字 + 单位
    ...
    # 中文数字 + 剂量单位（一两/三钱/二钱半/五克）
    re.compile(r'[一二三四五六七八九十百]+\s*(?:两|钱|克|毫\s*克|分)'),
    # 中文数字 + 服用频次（每日二次 / 一日两次 / 三次服用）
    re.compile(r'(?:每|一)?(?:日|天|次|服)\s*[一二三四五六七八九十]+\s*次'),
    re.compile(r'[一二三四五六七八九十]+\s*次\s*(?:服|用|进)'),
]
