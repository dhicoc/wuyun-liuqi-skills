...
def check_tong_tianfu(year):
    """同天符: 阳年（太过），中运五行 == 在泉五行"""
    dayun, _ = get_dayun(year)
    zaiquan = get_zaiquan(year)
    zaiquan_elem = LIUQI_WUXING[zaiquan]
    taiguo = is_taiguo(year)
    return taiguo and dayun == zaiquan_elem

def check_tong_suihui(year):
    """同岁会: 阴年（不及），中运五行 == 在泉五行"""
    dayun, _ = get_dayun(year)
    zaiquan = get_zaiquan(year)
    zaiquan_elem = LIUQI_WUXING[zaiquan]
    taiguo = is_taiguo(year)
    return (not taiguo) and dayun == zaiquan_elem

# ═══════════════════════════════════════════════════════════════
# 五运齐化兼化
# 依据：《素问·五常政大论》《医宗金鉴·运气要诀》五运齐化兼化歌
# 太过之运 -> 齐化：克我者来齐，如土太过则木来齐化
# 不及之运 -> 兼化：克我者来兼，如土不及则木来兼化
# ═══════════════════════════════════════════════════════════════

# 五行相克的反向映射：谁克我
_WUXING_KEME = {v: k for k, v in WUXING_KE.items()}

def get_qihua(year):
    """
    齐化: 太过之运，克我者来齐化。
    如土运太过，木来齐化（木克土，木气来齐）。
    返回: 齐化五行名 或 None（非太过之年）
    """
    dayun, _ = get_dayun(year)
    if not is_taiguo(year):
        return None
    return _WUXING_KEME.get(dayun)

def get_jianhua(year):
    """
    兼化: 不及之运，克我者来兼化。
    如土运不及，木来兼化（木克土，乘虚而入）。
    返回: 兼化五行名 或 None（非不及之年）
    """
    dayun, _ = get_dayun(year)
    if is_taiguo(year):
        return None
...
def get_zhengdui_huaqi(year):
    """
    正化对化: 根据年支判断司天之气的正化/对化。
    正化：气化之本位，气化有力；对化：气化之对位，气化较弱。
    返回: (六气名, 正化/对化)
    """
    _, dz = get_ganzhi(year)
    return DIZHI_ZHENGDUI.get(dz, (None, None))
