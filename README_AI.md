...
| 用户请求 | 推荐执行 |
|----------|----------|
| “算某日/今天运气” | `python scripts/calculate_yunqi_api.py today --summary`（支持 today，默认今天） |
| “给我 JSON / Agent 接口” | `python scripts/calculate_yunqi_api.py today --json` |
| “思想层 / 概念解释” | `... --level deep` / `--explain-concept "天人合一"` |
| “导出思想摘要/卡片/PDF” | `... --export summary|cards|pdf|all` 或 `scripts/export_thought.py today --format all` |
...
> 医案库合计 **1994 条**临证真实医案（asset9 圣济岁图 + asset11-32 历代名家库，含 herbs 药味 + formulas_referenced 方剂结构化字段）。按病证检索可经渐进加载路由 `cases_routing.py` 先拿候选库清单，再 `rag_search --asset <命中的库> <病证>` 按需加载，避免整包载入。
>
...
| 医案渐进加载路由 | `python scripts/cases_routing.py --syndrome <病证>` | 按病证/运气查医案库前先拿候选库清单（首选/补充/强制联动），再按需加载（P1-3） |
| 稳定引用解析 | `python scripts/resolve_ref.py <yle:引用>` | 反解 `yle:` 引用、核验出处可访问、批量可访问率门禁（P1-2） |
...
| [references/architecture.md](references/architecture.md) | 项目架构说明 |
| [references/roadmap.md](references/roadmap.md) | 后续路线图 |
