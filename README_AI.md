...
> 医案库合计 **2182 条**临证真实医案（asset9 圣济岁图 + asset11-32 历代名家库，含 herbs 药味 + formulas_referenced 方剂结构化字段）。按病证检索可经渐进加载路由 `cases_routing.py` 先拿候选库清单，再 `rag_search --asset <命中的库> <病证>` 按需加载，避免整包载入。
>
